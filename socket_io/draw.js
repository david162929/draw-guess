    ...
    draw.on("connection", (socket) => {
        ...
        socket.on("join-room", (lastRoomId, roomId) => {
            socket.leave(lastRoomId, () => {
                ...
                if (rooms[lastRoomId].clients.length === 1 && lastRoomId !== "room_1") {
                    ...
                    const clientIndex = findClient(socket.id, rooms[lastRoomId]);
                    rooms[lastRoomId].clients.splice(clientIndex, 1); // 刪除搜到的那項

                    const rankingIndex = findRanking(socket.id, rooms[lastRoomId]);
        ...
        socket.on("game-start", (roomId, userName) => {
            ...
            if (socket.id === rooms[roomId].gameDetail.currentDraw) {
                ...
                // 計算當前 topic
                const itemNum = rooms[roomId].gameDetail.orderNum % topic.length;
        ...
        socket.on("send-answer-message", (roomId, userName, msg) => {
            const itemNum = rooms[roomId].gameDetail.orderNum % topic.length; // 計算當前題目是題目組的第幾個
            if (msg === topic[itemNum] && rooms[roomId]) {
                ...
                if (alreadyCorrect === true) {
                    ...
                    // 找 rankingList
                    const rankingIndex = findRanking(socket.id, rooms[roomId]);
                    ...
                    const rankingIndex2 = findRanking(rooms[roomId].gameDetail.currentDraw, rooms[roomId]);
                    rooms[roomId].rankingList[rankingIndex2].score += 1;
                    rooms[roomId].rankingList.sort((a, b) => {
        ...
        socket.on("disconnect", () => {
            ...
            if (clients[socket.id]) {
                const disconRoom = clients[socket.id].room;
                ...
                if (rooms[disconRoom].clients.length === 1 && disconRoom !== "room_1") {
                    ...
                    const clientIndex = findClient(socket.id, rooms[disconRoom]);
                    rooms[disconRoom].clients.splice(clientIndex, 1); // 刪除搜到的那項
                    delete clients[socket.id]; // 刪除 clients 中的使用者

                    const rankingIndex = findRanking(socket.id, rooms[disconRoom]);
...
/**
 * 找到某玩家在 rooms[roomId] 中的 clients array 中的位置
 * @param {string} targetClientId 要找的玩家的 socket.id
 * @param {Object} targetRoomObj 當前的房間物件
 * @return {number|boolean} 存在就回傳 index 位置，不存在就回傳 false
 */
...
/**
 * 找到某玩家在 rooms[roomId] 中的 rankingList array 中的位置
 * @param {Object} targetClientId 要找的玩家的 socket.id
 * @param {Object} targetRoomObj 當前的房間物件
 * @return {number|boolean} 存在就回傳 index 位置，不存在就回傳 false
 */
...
/**
 * 遊戲的 middle phase 的計時器，時間到就觸發下一個事件
 * @param {Object} draw socket.io 物件的 namespace "/draw"
 * @param {Object} rooms 房間全域物件
 * @param {string} roomId 房間 id
 * @param {number} time middle phase 的總歷時
 */
function middlePhaseTimer(draw, rooms, roomId, time) {
    setTimeout(() => {
        if (rooms[roomId]) {
            ...
            userNum += 1;
            const socketId = rooms[roomId].clients[userNum] || rooms[roomId].clients[0];
